...
describe.only('test partial refund for Stripe gateway 3D and none 3D transactions', () => {
  const todaysDate = Cypress.moment().format('MM_DD_YYYY')
  const otherAmount = ('.en__field--12542 > .en__field__element > .en__field__item--other > .en__field__input')
  const donationSingleSepa = ('en_stripe_sepa_single_sepa_donation_' + todaysDate + '@tellamazingstories.com')
  const donationRecurSepa = ('en_stripe_sepa_recur_sepa_donation_' + todaysDate + '@tellamazingstories.com')
  const donationSingleCC3220 = ('en_stripe_partial_single_cc3220_donation_' + todaysDate + '@tellamazingstories.com')
  ...
  beforeEach(() => {
    cy.visit(Cypress.env('dev') + 'page/2910/donate/1')
  })
  ...
  it('it submits single sepa_debit transaction', () =>{
    ...
    cy.get(otherAmount).type('100.99')
...
context('Actions', () => {
  ...
  it('it submits single 3D transaction 4000000000003220', () => {
    ...
    cy.get('button').click()
    cy.url().should('eq', 'https://dev.politicalnetworks.com/page/2910/donate/2')
    cy.get('#en__field_transaction_donationAmt').select('Other').should('have.value', 'Other')
    cy.get(otherAmount).type('100.99')
    ...
    cy.location('pathname').should('include', 'page/2910/donate/2')
  ...
  it.only('it submits single 3D transaction 4000000000003063', () => {
  ...
  it('it submits single 3D card_decline transaction 4000008400001629', () => {
    ...
    // cy.wait(3000)
    // cy.waitForStripe3DIframe().find("[id*=test-source-authorize-3ds]").click()
    cy.get('.en__error').should('have.text', 'Error in getting the stripe payment intent.')
  ...
  function thankyouPage() {
    cy.location('pathname').should('include', 'page/2910/donate/3')
    cy.get('.en__component--column > .en__component').as('thankYouPage')
    cy.get('@thankYouPage').contains('8165')
...
it('searches for the recurring donation sepa_debit transaction and completes partial refund', () => {
 ...
 cy.get('.gadget__transactionHistory__transaction__field__type').eq(0).invoke('text')
  ...
  cy.get('.gadget__transactionHistory__transaction__field__type').eq(1).click()
