describe('test partial refund for Iats gateway for event tickets', ()=>{
    ...
    it('can purchase and validate tickets ', () =>{
      ...
      cy.visit(Cypress.env('test')+'page/13216/event/1')
        ...
        cy.url().should('include', '/13216/event/2')
        cy.get('.en__orderSummary__data--totalAmount').should('have.text', '431.44 GBP')
        ...
        cy.get('button').click()
        cy.url().should('include', '/13216/event/3')
    ...
    it('can purchase and validate tickets with discount', () =>{
      ...
      cy.visit(Cypress.env('test')+'page/13216/event/1')
        ...
        cy.get('.en__ticketSummary__checkout').click({ delay: 2000 })
        cy.url().should('include', '/13216/event/2')
        cy.get('.en__orderSummary__data--totalAmount').should('have.text', '391.44 GBP')
    ...
    cy.get('.gadget__receipt > p').invoke('text').should('contain', 'Amount Charged: 431.44 GBP')
    cy.get('.gadget__events__table').find('tr').within(() => {
        cy.get('td').eq(0).find('input').should('have.value', '4157').check()
        cy.get('td').eq(1).should('have.text', 'Ticket')
        cy.get('td').eq(3).should('have.text', '10.99 GBP')
        cy.get('td').eq(5).should('have.text', 'Ticket')
        cy.get('td').eq(7).should('have.text', '10.99 GBP')
        cy.get('td').eq(8).find('input').should('have.value', '4159').check()
        cy.get('td').eq(9).should('have.text', 'VIP')
        cy.get('td').eq(11).should('have.text', '25.99 GBP')
        cy.get('td').eq(13).should('have.text', 'VIP')
        cy.get('td').eq(15).should('have.text', '25.99 GBP')
        cy.get('td').eq(16).find('input').should('have.value', '4158').check()
        cy.get('td').eq(17).should('have.text', 'Group')
        cy.get('td').eq(23).should('have.text', '100.99 GBP')
        cy.get('td').eq(25).should('have.text', 'Group')
        cy.get('td').eq(31).should('have.text', '100.99 GBP')
        cy.get('td').eq(35).should('include.text', '155.5 GBP')
    ...
    cy.get('@refund').should('include', '-137.97 GBP')
    cy.get(event).eq(1).click()
    cy.get('.gadget__attachment__view').should('be.visible')
    cy.get('.gadget__attachment__resend').should('be.visible')
    cy.get('.gadget__attachment__refund').click()
    cy.get('.gadget__receipt > p').invoke('text').should('contain', 'Amount Charged: 293.47 GBP')
    cy.get('.gadget__events__table').find('tr').within(() => {
...
it('refunds only partial additional amount', () => {
  ...
  logIn()
    ...
    cy.get('.gadget__singleDonations__transaction').invoke('text').as('refund')   
    cy.get('@refund').should('include', '-19.99 GBP')
    cy.get(event).eq(2).click()
    cy.get('.gadget__attachment__view').should('be.visible')
    cy.get('.gadget__attachment__resend').should('be.visible')
    cy.get('.gadget__attachment__refund').click()
    cy.get('.gadget__receipt > p').invoke('text').should('contain', 'Amount Charged: 273.48 GBP')
...
it('refunds partial additional amount and all tickets', () => {
  ...
  logIn()
    ...
    cy.get('.gadget__events__table').find('tr').within(() => {
        ...
        cy.get('td').eq(39).should('include.text', '19.99 GBP')    
    ...
    cy.get(event).eq(0).click().trigger('mouseover')
    cy.get('.gadget__singleDonations__transaction').invoke('text').as('refund')   
    cy.get('@refund').should('include', '-273.48 GBP')
...
it('refunds tickets with discount code', () => {
  ...
  cy.get('.enDashboard__gadget__content > form > .userInput > .userInput__field > input')
    ...
    cy.get('.gadget__attachment__resend').should('be.visible')
    cy.get('.gadget__attachment__refund').click()
    cy.get('.gadget__receipt > p').invoke('text').should('contain', 'Amount Charged: 391.44 GBP')
    cy.get('.gadget__events__table').find('tr').within(() => {
      cy.get('td').eq(0).should('have.text', 'Ticket')
      cy.get('td').eq(2).should('have.text', '10.99 GBP')
      cy.get('td').eq(3).should('have.text', 'Ticket')
      cy.get('td').eq(5).should('have.text', '10.99 GBP')
      cy.get('td').eq(6).should('have.text', 'VIP')
      cy.get('td').eq(8).should('have.text', '15.99 GBP')
      cy.get('td').eq(9).should('have.text', 'VIP')
      cy.get('td').eq(11).should('have.text', '15.99 GBP')
      cy.get('td').eq(12).should('have.text', 'Group')
      cy.get('td').eq(18).should('have.text', '90.99 GBP')
      cy.get('td').eq(19).should('have.text', 'Group')
      cy.get('td').eq(25).should('have.text', '90.99 GBP')
      cy.get('td').eq(28).should('include.text', '155.5 GBP')
  ...
  cy.get(event).eq(0).click().trigger('mouseover')
  cy.get('.gadget__singleDonations__transaction').invoke('text').as('refund')   
  cy.get('@refund').should('include', '-391.44 GBP')
  cy.get(event).eq(1).click()
  cy.get('.gadget__attachment__view').should('be.visible')
  cy.get('.gadget__attachment__resend').should('be.visible')
  cy.get('.gadget__attachment__refund').click()
  cy.get('.gadget__receipt > p').invoke('text').should('contain', 'Amount Charged: 0 GBP')
  cy.get('td > .gadget__receipt__field').should('contain.text', '391.44 GBP')
