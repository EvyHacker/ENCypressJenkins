describe('test partial refund for Iats gateway for event tickets', ()=>{
    ...
    const ticket = ('en_iats_partial_events_' + todaysDate + '@engagingnetworks.online')
    const ticketDiscount = ('en_iats_partial_event_discount_' + todaysDate + '@engagingnetworks.online')
    ...
    it('can purchase and validate tickets ', () =>{
      ...
      cy.visit(Cypress.env('test')+'page/13215/event/1')
        ...
        cy.url().should('include', '/13215/event/2')
        ...
        cy.get('button').click()
        cy.url().should('include', '/13215/event/3')
    ...
    cy.get('.gadget__events__table').find('tr').within(() => {
        cy.get('td').eq(0).find('input').should('have.value', '4154').check()
        ...
        cy.get('td').eq(8).find('input').should('have.value', '4155').check()
        ...
        cy.get('td').eq(16).find('input').should('have.value', '4156').check()
